    ...
    expected = ValidationErrorResponseSchema(
        details=[
            ValidationErrorSchema(
                ...
                input="incorrect-file-id",  # Не uuid.
                context={"error": "invalid character: expected an optional prefix of `urn:uuid:` followed by [0-9a-fA-F-], found `i` at 1"},  # Ожидаем uuid.
                ...
                location=["path", "file_id"]  # Ошибка возникает в path, поле "file_id".
            )
        ]
    )
    assert_validation_error_response(actual, expected)